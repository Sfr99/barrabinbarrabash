...
def format_datetime(dt: datetime) -> str:
    """Formats a datetime into a UI-friendly string."""
    return dt.strftime("%Y-%m-%d %H:%M:%S")


def format_api_datetime(dt_str: str) -> str:
    """
    Parses an ISO8601 datetime returned by the FastAPI backend
...
@app.route("/")
def dashboard():
    # Fetch full system state (banned IPs, events, today's attacks)
...
@app.route("/bans")
def bans():
    # Reuse the /state endpoint to get banned IPs
    state_res = requests.get(f"{API_BASE_URL}/state")
    state_res.raise_for_status()
    state = state_res.json()
...
def unban(ip):
    # Request backend to unban this IP via POST /unban/{ip}
    res = requests.post(f"{API_BASE_URL}/unban/{ip}")
    # check for errors with res.status_code
    return redirect(url_for("bans"))


@app.route("/reset", methods=["POST"])
def reset():
    # call FastAPI backend: POST /reset
